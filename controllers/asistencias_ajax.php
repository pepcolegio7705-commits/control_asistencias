...
switch ($action) {
    case 'obtener_planilla':
        ...
        $stmtArt = $pdo->query("SELECT id, codigo, descripcion FROM articulos WHERE estado = 1 ORDER BY codigo");
        ...
        for ($d = 1; $d <= $dias_en_mes; $d++) {
            ...
            $html .= "<th class='text-center {$bg}'>{$d}</th>";
        }
    ...
    case 'obtener_articulos':
        $stmt = $pdo->query("SELECT id, codigo, descripcion FROM articulos WHERE estado = 1 ORDER BY codigo");
        $data = [];
