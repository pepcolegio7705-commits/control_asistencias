...
switch ($action) {
    case 'obtener_planilla':
        ...
        $stmtProf = $pdo->query("SELECT id, nombre, apellido FROM profesores WHERE estado = 1 ORDER BY apellido, nombre");
        $profesores = $stmtProf->fetchAll();
        
        // Obtener artículos para el desglose del popover
        $stmtArt = $pdo->query("SELECT id, codigo, descripcion FROM articulos WHERE estado = 1 AND sector IN ('docente', 'ambos') ORDER BY codigo");
        ...
        $stmtAsis = $pdo->prepare("
            SELECT a.id as asis_id, a.profesor_id, a.fecha, a.articulo_id, ar.codigo 
            FROM asistencias a
        ...
        // Mapear asistencias por profesor y dia
        $mapa_asistencias = [];
        $totales_profesor = []; // Para los contadores finales del mes
        
        foreach ($asistenciasRaw as $row) {
            $dia = (int)date('j', strtotime($row['fecha']));
            $mapa_asistencias[$row['profesor_id']][$dia] = [
                'asis_id' => encrypt_id($row['asis_id']),
                'codigo' => $row['codigo'],
                'articulo_id' => encrypt_id($row['articulo_id'])
            ];
            
            if(!isset($totales_profesor[$row['profesor_id']][$row['codigo']])) {
                $totales_profesor[$row['profesor_id']][$row['codigo']] = 0;
            }
            $totales_profesor[$row['profesor_id']][$row['codigo']]++;
        }
        ...
        $html .= '<th style="min-width: 200px;">Profesor</th>';
        ...
        foreach ($profesores as $prof) {
            ...
            $html .= "<tr>";
            $html .= "<td class='fw-bold text-nowrap'>" . htmlspecialchars($prof['apellido'] . ', ' . $prof['nombre']) . "</td>";
            ...
            $totalFaltas = 0;
            $desgloseItems = [];
            if (isset($totales_profesor[$prof['id']])) {
                foreach ($totales_profesor[$prof['id']] as $cod => $cant) {
                    $totalFaltas += $cant;
    ...
    case 'obtener_anual':
        ...
        $stmtAsis = $pdo->prepare("
            SELECT a.profesor_id, ar.codigo, COUNT(a.id) as cantidad
            FROM asistencias a
            JOIN articulos ar ON a.articulo_id = ar.id
            WHERE a.fecha BETWEEN ? AND ?
            GROUP BY a.profesor_id, ar.codigo
        ");
        $stmtAsis->execute([$fecha_inicio, $fecha_fin]);
        
        $totales = [];
        while($row = $stmtAsis->fetch()) {
            $totales[$row['profesor_id']][$row['codigo']] = $row['cantidad'];
        }
        ...
        // Obtener totales por mes por profesor
        $stmtMensual = $pdo->prepare("
            SELECT a.profesor_id, MONTH(a.fecha) as mes, COUNT(a.id) as cantidad
            FROM asistencias a
            WHERE a.fecha BETWEEN ? AND ?
            GROUP BY a.profesor_id, MONTH(a.fecha)
        ");
        $stmtMensual->execute([$fecha_inicio, $fecha_fin]);
        $totalesMensuales = [];
        while($row = $stmtMensual->fetch()) {
            $totalesMensuales[$row['profesor_id']][$row['mes']] = $row['cantidad'];
        }
        ...
        foreach ($profesores as $prof) {
            $html .= "<tr>";
            $html .= "<td class='fw-bold'>" . htmlspecialchars($prof['apellido'] . ', ' . $prof['nombre']) . "</td>";
            
            $totalAnual = 0;
            foreach ($meses as $numMes => $nombreMes) {
                $cantMes = $totalesMensuales[$prof['id']][$numMes] ?? 0;
                $totalAnual += $cantMes;
                if ($cantMes > 0) {
            ...
            $desgloseItems = [];
            if (isset($totales[$prof['id']])) {
                foreach ($totales[$prof['id']] as $cod => $cant) {
                    $desc = isset($articulosMap[$cod]) ? htmlspecialchars($articulosMap[$cod]) : $cod;
                    $desgloseItems[] = "<div class='d-flex justify-content-between'><span class='me-3'><strong>{$cod}</strong> - {$desc}</span><span class='badge bg-danger'>{$cant}</span></div>";
                }
            }
    ...
    case 'obtener_articulos':
        $stmt = $pdo->query("SELECT id, codigo, descripcion FROM articulos WHERE estado = 1 AND sector IN ('docente', 'ambos') ORDER BY codigo");
        $data = [];
    ...
    case 'guardar':
        ...
        $profesor_id = decrypt_id($_POST['profesor_id'] ?? '');
        $fecha = $_POST['fecha'] ?? '';
        $articulo_id = decrypt_id($_POST['articulo_id'] ?? '');
        
        if (!$profesor_id || empty($fecha) || !$articulo_id) {
        ...
        try {
            if ($asis_id) {
                ...
                $stmt = $pdo->prepare("UPDATE asistencias SET articulo_id = ? WHERE id = ?");
                $stmt->execute([$articulo_id, $asis_id]);
                echo json_encode(['status' => 'success', 'msg' => 'Asistencia actualizada.']);
            } else {
                // Prevenir duplicados (idx_profesor_fecha)
                $stmt = $pdo->prepare("INSERT INTO asistencias (profesor_id, fecha, articulo_id) VALUES (?, ?, ?)");
                $stmt->execute([$profesor_id, $fecha, $articulo_id]);
                echo json_encode(['status' => 'success', 'msg' => 'Artículo cargado correctamente.']);
            }
        } catch (PDOException $e) {
            if ($e->getCode() == 23000) {
                echo json_encode(['status' => 'error', 'msg' => 'Ya existe un artículo cargado para este docente en esta fecha.']);
            } else {
    ...
    case 'eliminar':
        ...
        if ($asis_id) {
            ...
            if ($stmt->execute([$asis_id])) {
                echo json_encode(['status' => 'success', 'msg' => 'Artículo eliminado. El docente vuelve a figurar Presente.']);
            } else {
