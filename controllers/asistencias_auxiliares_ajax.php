        ...
        foreach ($auxiliares as $aux) {
            ...
            for ($d = 1; $d <= $dias_en_mes; $d++) {
                ...
                if (isset($mapa_asistencias[$aux['id']][$d])) {
                    ...
                    $onclick = $isUsuario ? "" : "style='cursor:pointer;' onclick='abrirModalAsistencia(\"{$aux_enc_id}\", \"{$fecha_actual}\", \"{$asis['asis_id']}\", \"{$asis['articulo_id']}\")'";
                    $html .= "<td class='text-center {$td_class}' {$onclick}>";
                    $html .= "<span class='badge bg-danger'>{$asis['codigo']}</span>";
                    $html .= "</td>";
                } else {
                    // Presente (celda vacía)
                    $onclick = $isUsuario ? "" : "style='cursor:pointer;' onclick='abrirModalAsistencia(\"{$aux_enc_id}\", \"{$fecha_actual}\", \"\", \"\")'";
